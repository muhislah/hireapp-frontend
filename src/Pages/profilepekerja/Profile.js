      ...
      <div className={style.main}>
        ...
        >
          ...
          <div className={style.profile}>
            <img
              src={
                data.employee.length > 0 ? data.employee[0].image_profil : photo
              }
              alt="photoprofile"
            />
            <p id="name" className="h4 mt-3 font-weight-bold">
            ...
            <p className="text-muted w-50">
              {data.employee.length > 0 && data.employee[0].description}
            </p>
            <Link
              to={`/Hire/${
                data.employee.length > 0 && data.employee[0].idemployee
              }`}
            >
          ...
          <div className={style.skills + " w-25"}>
            {data.employee.length > 0
              ? data.employee[0].skill.map((data) => (
                  <div className={style.skill}>{data}</div>
                ))
              : "Not Skill Found"}
          </div>
          <div className={style.account + " w-25 mt-4"}>
            <div className="mb-2">
              <img src={mail} alt="" />
              <span className="text-muted ml-3">
                {data.employee.length > 0 && data.employee[0].email}
              </span>
            </div>
            <div className="mb-2">
              <img src={instagram} alt="" />
              <span className="text-muted ml-3">
                {data.employee.length > 0 && data.employee[0].instagram}
              </span>
            </div>
            <div className="mb-2">
              <img src={github} alt="" />
              <span className="text-muted ml-3">
                {data.employee.length > 0 && data.employee[0].github}
              </span>
            </div>
          </div>
          <div className={style.menu + " mt-5"}>
            <button
              className={active === "portfolio" ? style.active : ""}
              onClick={() => {
                setActive("portfolio");
                navigate("/employee/" + idemployee + "/portfolio");
              }}
            >
              Portfolio
            </button>
            <button
              ...
              onClick={() => {
                ...
                navigate("/employee/" + idemployee + "/experience");
              }}
            >
              Pengalaman Kerja
            </button>
